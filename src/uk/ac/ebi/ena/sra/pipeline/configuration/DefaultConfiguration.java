...
ConfigurationException extends RuntimeException
{
	...
	public 
	ConfigurationException( String value ) 
	{
		super( value );
	}
	
	public 
	ConfigurationException( Throwable value ) 
	{
		super( value );
	}
...
DefaultConfiguration 
{
	...
	private void 
	checkProperties() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException
	{
	...
	getProperty( String key ) throws ConfigurationException
	{
		...
		if( !p.containsKey( e_key ) )
			throw new ConfigurationException( String.format( "FATAL: Property key %s for mode %s file[%s] not found!",
					                                     	 e_key, 
					                                     	 prefix, 
					                                     	 f_name ) );
	...
	getDefaultLSFUser()
	{
		return getProperty( "default.lsf-user" );
	}

    
    @PipeliteProperty public int
	...
	createConnection() throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException
	{
		...
		Connection connection = new OracleHeartBeatConnection( DriverManager.getConnection( getJDBCUrl(), props ) );
    ...
    //@PipeliteProperty
    public String
    getUserParameterTableName()
    {
        return getProperty( "user.parameters.table.name" );
    }
    ...
    /* TODO: remove
    pipelite.tables.pipeline.column.name=PIPELINE
    pipelite.tables.process.column.name=PROCESS_ID
    pipelite.tables.stage.column.name=STAGE_NAME
    */
			...
			lock:   while( true )
			{
				for( Object iface : ifaces )
					if( iface == iface_klass )
						break lock;
				throw new ConfigurationException( "Stages class should implement " + iface_klass.getName() + " interface" );
			}
	...
	@PipeliteProperty public ExecutionResult[]
	getCommitStatus()
	{
