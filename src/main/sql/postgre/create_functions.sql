 ...
 */
create or replace function pipelite2_init_or_reset(
    v_pipeline_name in pipelite2_stage.pipeline_name%type,
    v_process_id in pipelite2_stage.process_id%type,
    v_priority in pipelite2_process.priority%type default 5)
returns void
...
begin
    ...
    if v_init
    then
        perform pipelite2_init(v_pipeline_name, v_process_id, v_priority);
    else
        perform pipelite2_reset(v_pipeline_name, v_process_id);
    end if;
end;
$$ LANGUAGE plpgsql;
