...
public class
OracleTaskIdSource2 implements OracleCommons, TaskIdSource
{
    ...
    prepareQuery()
    {
        ...
        if( redo.length() <= 2 )
            redo.delete( 0, redo.length() ).append( "''" );
        else
            redo.setLength( redo.length() - 2 );

        if( terminal.length() <= 2 )
            terminal.delete( 0, terminal.length() ).append( "''" );
        else
            terminal.setLength( terminal.length() - 2 );
...
/*11*/          getRedoCount() );
    ...
    public void
    setCommitStatus( ExecutionResult commit_status[] )
    {
