...
/** Manages locks to prevent activities from being executed more than once at the same time. */
public abstract class LockManager {
  ...
  public interface LockCallback {
     ...
     *
     * @param exception thrown during lock refresh.
     */
  ...
  public static final int DEFAULT_LOCK_REFRESH_INTERVAL = 1 * 60 * 1000; // 1
  // minutes

  /** The default lock expiry interval in milliseconds. */
  public static final int DEFAULT_LOCK_EXPIRED_INTERVAL = 5 * 60 * 1000; // 5
  // minutes
  ...
  /**
   * @param refreshInterval The lock refresh interval in milliseconds.
   * @param expiredInterval The lock expiry interval in milliseconds.
   */
  public LockManager(LockCallback lockCallback, int refreshInterval, int expiredInterval) {
  ...
  private void refreshLocks() throws LockException {
   ...
   * @throws LockException In case of any error.
   */
  protected abstract void refreshLock(String lock) throws LockException;
   ...
   * @throws LockException In case of any error.
   */
  public final boolean lock(String lock) throws LockException {
   ...
   *
   * @param lock The lock identifier.
   * @param force If true then any existing lock will be removed.
   * @return True if the lock was created. False if the lock already existed.
   * @throws LockException In case of any error.
   */
  public final boolean lock(String lock, boolean force) throws LockException {
    if (!locks.contains(lock)) {
      locks.add(lock);
    }
  ...
  protected abstract boolean _lock(String lock, boolean force) throws LockException;
   ...
   * @throws LockException In case of any error.
   */
  public abstract boolean isLocked(String lock) throws LockException;

  /**
   * Unlocks a stage instance.
   *
   * @param lock The lock to be released.
   * @return True if the lock was released. False if the lock had already been released.
   * @throws LockException In case of any error.
   */
  public final boolean unlock(String lock) throws LockException {
    locks.remove(lock);
    return _unlock(lock);
  }
  ...
  public int getLockRefreshInterval() {
    return refreshInterval;
  }

  /** @return The lock expiry interval in milliseconds. */
  public int getLockExpiredInterval() {
  ...
  /** @param lockRefresh If true then the locks will be periodically refreshed. */
