...
public interface ProcessRepository extends CrudRepository<ProcessEntity, ProcessEntityId> {
  ...
  @Query(
      value =
          "SELECT * FROM PIPELITE2_PROCESS A WHERE PIPELINE_NAME = ?1 AND STATE = 'ACTIVE' AND NOT EXISTS (SELECT 1 FROM PIPELITE2_PROCESS_LOCK B JOIN PIPELITE2_SERVICE_LOCK C USING (SERVICE_ID) WHERE A.PIPELINE_NAME = B.PIPELINE_NAME AND A.PROCESS_ID = B.PROCESS_ID AND C.EXPIRY > ?2) ORDER BY PRIORITY DESC",
      nativeQuery = true)
